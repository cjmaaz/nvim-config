...
| Indent guides | `lua/plugins/indent.lua` | Dim passive `│` guides; **bright current scope** (same 10 colors as rainbow brackets) with **reverse-L** underlines (`show_start` / `show_end`). Palette: `lua/config/rainbow_palette.lua`. |
...
**Scope look:** under the cursor’s block you get a stronger vertical bar plus underlines at the block’s first/last line (reverse-L). Passive off underlines or switch to a single `IblScope` color in `indent.lua` comments.
