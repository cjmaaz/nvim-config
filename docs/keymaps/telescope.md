...
Panel chrome uses the shared dark Bordo palette from `lua/config/ui_chrome.lua`: soft-white borders, a raised active row, and yellow/rose accents matching the bars, which-key, and neo-tree. Normal pickers use a responsive 90% × 85% horizontal grid with separately rounded prompt/results/preview boxes, the prompt at the bottom, and the selected file or symbol in the preview title. File rows and picker markers use Nerd Font icons when available, with plain-text fallbacks.
