...
Popup: **classic** full-width strip with a rounded subtle border and Rosé Pine surface bg from `lua/config/ui_chrome.lua` (shared across bars and panels), max height **50%** of the screen, `no_overlap = false`. Columns have no forced maximum width, so long descriptions stay intact rather than being ellipsized.
